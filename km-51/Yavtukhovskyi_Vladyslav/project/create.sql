...
(
   ...
   name_of_medicine     VARCHAR2(20)         not null,
   constraint PK_MEDICINE_IS_FOR_WEAKNESS primary key (name_of_weakness, name_of_medicine)
);
...
(
   name_of_syndrom      VARCHAR2(20)         not null,
   name_of_weakness     VARCHAR2(20)         not null,
   constraint PK_WEAKNESS_HAS_SYNDROM primary key (name_of_syndrom, name_of_weakness)
);
...
(
   ...
   num_of_record        INTEGER              not null,
   date_of_begin        DATE,
   date_of_end          DATE,
...
alter table Medicine_is_for_weakness
   add constraint FK_MEDICINE_MEDICINE__MEDICINE foreign key (name_of_medicine)
      references Medicine (name_of_medicine);

alter table Medicine_is_for_weakness
   add constraint FK_MEDICINE_WEAKNESSS_WEAKNESS foreign key (name_of_weakness)
      references Weakness (name_of_weakness);

alter table Weakness_has_syndrom
   add constraint FK_WEAKNESS_WEAKNESS__SYNDROME foreign key (name_of_syndrom)
      references Syndromes (name_of_syndrom);

alter table Weakness_has_syndrom
   add constraint FK_WEAKNESS_WEAKNESS__WEAKNESS foreign key (name_of_weakness)
      references Weakness (name_of_weakness);

alter table healing_histories
   add constraint FK_HEALING__SYNDROMES_SYNDROME foreign key (name_of_syndrom)
      references Syndromes (name_of_syndrom);

alter table healing_histories
   add constraint FK_HEALING__USED_MEDI foreign key (name_of_weakness, name_of_medicine)
      references Medicine_is_for_weakness (name_of_weakness, name_of_medicine);

alter table healing_histories
   add constraint FK_HEALING__USER_HAS__USERSS foreign key (login)
...
BEGIN
logi := :new.login;
SELECT COUNT(*) INTO num_exist FROM HEALING_HISTORIES WHERE login = logi;
